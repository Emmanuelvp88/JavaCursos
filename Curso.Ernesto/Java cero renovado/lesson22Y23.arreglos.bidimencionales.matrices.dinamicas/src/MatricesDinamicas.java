    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < matriz.length; i++) {//recorre filas
            for (int j = 0; j < matriz[i].length; j++) {//recorre columnas, matriz[i] le dice que recorra los elementos de la fila "i"
                System.out.println("dame el valor de la posición fila " + (i + 1) + " columna " + (j + 1));
                matriz[i][j] = sc.nextInt();//para que el usuario ingrese manualmente los números que desea que contenga el array
            }
        }
